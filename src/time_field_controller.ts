...
export type TimeFieldChangeDetail = {
	value: string;
...
/**
 * Adds cancelable user changes and state outputs to a native time input.
 *
 * @see https://github.com/tknf/stimulus-ui/blob/main/design/contracts/time-field.contract.json
 */
export default class TimeFieldController extends Controller<HTMLElement> {
	...
	private nativeBoundInput: HTMLInputElement | null = null;
	...
	/**
	 * Native time input value in HH:MM, HH:MM:SS, or HH:MM:SS.sss form, or an empty string for
	 * missing or invalid input. Assignment synchronizes value and data-state without custom events.
	 */
	get value(): string {
	...
	private nativeInput = () =>
		this.element instanceof HTMLInputElement && this.element.type === "time" ? this.element : null;
	...
	private commitChange = (input: HTMLInputElement, pending: PendingInteraction) => {
		const detail: TimeFieldChangeDetail = {
			value: input.value,
			previousValue: pending.previousValue,
			reason: pending.reason,
		};
		const beforeChange = new CustomEvent<TimeFieldChangeDetail>("time-field:beforechange", {
			bubbles: true,
			cancelable: true,
			detail,
		});
		...
		this.element.dispatchEvent(
			new CustomEvent<TimeFieldChangeDetail>("time-field:change", {
				bubbles: true,
				cancelable: false,
				detail,
			}),
	...
	private nativeBound = (rawValue: string) => {
		...
		const input =
			this.nativeBoundInput ??
			(this.nativeBoundInput = this.element.ownerDocument.createElement("input"));
		input.type = "time";
